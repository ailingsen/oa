...
//			if(cld[sD].isToday) $("#GD" + i).addClass("jinri"); //今日颜色
    ...
    //初始化工作日数据--------------------------------------------------------------------------------------------------------------------
    var date = global.currYear.toString()+'-'+(global.currMonth+1).toString()+'-1';
    dateSelection.getMonthWorkday(date);
}
...
function mOck(thisObj, v) {
    ...
    setdate = nian +'-'+ addZ(yue) +'-'+addZ(day);
	if(onoff == '0') {
		//thisObj.style.background='#FBBB67';不用
        //设置工作日---------------------------------------------------------------------------------------------
        dateSelection.setWorkday(onoff, thisObj, nian, yue, day, setdate);
		/*thisObj.setAttribute("class", "selday");
		thisObj.attributes["on"].value = '1';
		hDays.push(dayJson);
		curDay=nian + addZ(yue) + addZ(day);*/
	} else {
        //取消设置工作日---------------------------------------------------------------------------------------------
        dateSelection.setWorkday(onoff, thisObj, nian, yue, day, setdate);
		//thisObj.style.background='';不用
...
$(function() {
	...
	$("#yuejian").click(function() {
        ...
        /*console.log(global.currYear);
        console.log(global.currMonth);*/
	});
...
//		$('#SM' + this.currMonth).addClass('curr');
    ...
    //根据时间获取当月的工作日-----------------------------------------------------------------------------------------------------------
    getMonthWorkday: function(date) {
        $.ajax({
            type:"POST",
            url:"/index.php?r=management/holiday/month-workday",
            data:{date:date},
            //返回数据的格式
...
//                  	console.log((initDay[0]+parseInt(list[i].sun))-1);
                    	$("#GD"+((initDay[0]+parseInt(list[i].sun))-1)).attr('on',"1");
                        $("#GD"+((initDay[0]+parseInt(list[i].sun))-1)).addClass("selday");
                    }
                }
                initDay = [];
            },
    ...
    //根据时间获取当月的工作日-----------------------------------------------------------------------------------------------------------
    setWorkday: function(onoff, thisObj, nian, yue, day, date) {
        var temponoff = onoff==1 ? 0:1;
        $.ajax({
            type:"POST",
            url:"/index.php?r=management/holiday/set-workday",
            data:{date:date,status:temponoff},
            //返回数据的格式
            datatype: "json",//"xml", "html", "script", "json", "jsonp", "text".
            //在请求之前调用的函数
            beforeSend:function(){
...
关联layout css3 动画
*/ 
var isalertIdshow = true, outTimes = 1000;
function alert(str){
