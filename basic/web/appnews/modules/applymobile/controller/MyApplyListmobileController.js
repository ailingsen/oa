...
ApplymobileMod.controller('ApplyMobileController',function($scope,$http,$state,$rootScope,$stateParams){
    $("#mLayer2").show();
})

ApplymobileMod.controller('MyApplyListmobileController',function($scope,$http,$state,$rootScope,$stateParams,Publicfactory,applymobileModel,leaveApplyModel,noticeModel,filtersModel,checkModel,formshowfactory,permissionService){
    ...
    //获取审批单名称选项
    apply.modelList = [];
    applymobileModel.getApplyModel($scope);
    //显示状态选中框
    ...
    //获取我的申请列表
    applymobileModel.getMyApplyList($scope);
    ...
    apply.doDel = function() {
        applymobileModel.delApply(apply.delApplyId,$scope);
        return;
    ...
    apply.doRevoke = function() {
        applymobileModel.revoke(apply.revokeApplyId,$scope);
        return;
        apply.rankPopup = false;
    ...
    apply.press = function(apply_id) {
        applymobileModel.press(apply_id,$scope);
        return;
        apply.showRevokeWin = false;
    ...
    apply.formSave = function () {
        if(formshowfactory.formshowsave(JSON.parse($scope.apply.detail.form_json))){
            ...
            applymobileModel.applyEdit($scope, closeWin);
        }
    }
    ...
    apply.toEditApply = function(apply_id,model_id) {
        window.location.href="/index_news.html#/applymobile/mine/"+apply_id+"/0/1"
    }

    //编辑
    apply.editApply = (function(apply_id,model_id){
        if (!parseInt($stateParams.action_type)) {
        ...
        if(model_id == 2) {
            ...
            if ($stateParams.action_type == 1) {
                applymobileModel.applyDetail($scope,apply_id,1);
    ...
    })($stateParams.apply_id,$stateParams.model_id)
    ...
    apply.detailApply = (function(apply_id,model_id){
        ...
        if(model_id == 2) {
            ...
            if ($stateParams.action_type == 0) {
                applymobileModel.applyDetail($scope,apply_id,0);
                applyEditParam.apply_id = apply_id;
            }
            // switch (model_id){
            //     case 1://加班申请弹出窗
